...
function (angular, app, $, _, kbn, moment, timeSeries, numeral) {
  ...
  var module = angular.module('kibana.panels.simhistogram', []);
  app.useModule(module);

  module.controller('simhistogram', function($scope, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
      ...
      editorTabs : [
        {
          title:'Style',
          src:'app/panels/simhistogram/styleEditor.html'
        },
        {
          title:'Queries',
          src:'app/panels/simhistogram/queriesEditor.html'
        },
      ],
      status  : "Stable",
    ...
    $scope.get_data = function(data, segment, query_id) {
      ...
      _.each(queries,function(q) {
        ...
        if (old_query && !isNaN(old_query)) {
          ...
          bugid = q.query;
          q.query = '_type:similarities AND (id1:' + bugid + 
	    ' OR id2:' + bugid + ')';
	  q.alias = old_query;
        }
